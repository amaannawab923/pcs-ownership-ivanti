...
export interface HydrateChartData {
  /**
   * Present for every chart. `slice_id`/`form_data` are omitted by
   * `/dashboard/<id>/charts` for a chart the viewer has no access to, so this
   * is the only identifier such an entry carries.
   */
  id?: number;
  /** Set during hydration when the API returned no form_data: the viewer has
   *  no access to this chart, so its tile renders a placeholder. */
  noAccess?: boolean;
  /** False when the API determined the viewer cannot access this chart. */
  has_access?: boolean;
  /** Owner display names, returned alongside `has_access: false` so a denied
   *  tile can name someone to ask for access. */
  owners?: string[];
  slice_id: number;
  slice_url: string;
  slice_name: string;
...
export const hydrateDashboard =
  ...
  (dispatch: AppDispatch, getState: GetState): AnyAction => {
    ...
    charts.forEach((chartItem: HydrateChartData) => {
      // A chart the viewer has no access to comes back from
      // `/dashboard/<id>/charts` WITHOUT `form_data` (and without `slice_id`),
      // carrying only `id`/`slice_name`. Dereferencing `form_data` here took
      // the ENTIRE dashboard down with "Cannot read properties of undefined
      // (reading 'slice_id')" -- one restricted chart blanked every other
      // chart on the page.
      //
      // Substitute the minimum hydration needs so the dashboard renders and
      // the tile stays in its original grid position (position_json is never
      // modified); the tile itself then shows an access placeholder.
      // `has_access: false` is the API saying, explicitly, that this viewer
      // cannot see this chart -- it also omits `form_data`. Keying off the
      // marker (not the absence of form_data) is what lets the tile explain
      // itself instead of guessing.
      if (chartItem.has_access === false || !chartItem.form_data) {
        // eslint-disable-next-line no-param-reassign
        chartItem.noAccess = true;
        // Minimum needed for hydration to map the tile to its layout slot.
        // position_json is never modified, so the tile keeps its place.
        // eslint-disable-next-line no-param-reassign
        chartItem.form_data = chartItem.form_data ?? { slice_id: chartItem.id };
      }
      // eslint-disable-next-line no-param-reassign
      chartItem.slice_id = (chartItem.form_data.slice_id ??
        chartItem.id) as number;
    });
