...
import { ErrorTypeEnum, VizType } from '@superset-ui/core';
...
import { ErrorMessageWithStackTrace } from 'src/components/ErrorMessage';
...
const Chart = (props: ChartProps) => {
  ...
  // The viewer cannot access this chart: the dashboard API returned
  // `has_access: false` for it. The tile keeps its position in the layout AND
  // its header -- the title is how a viewer refers to the chart when asking
  // for access -- and its body is replaced by the standard chart-access error
  // instead of a data request it could never complete.
  const noAccess = (slice as JsonObject).noAccess === true;
  const noAccessError: SupersetError | null = noAccess
    ? {
        error_type: ErrorTypeEnum.CHART_SECURITY_ACCESS_ERROR,
        level: 'error',
        message: '',
        extra: {
          chart_id: slice.slice_id,
          slice_name: slice.slice_name,
          owners: (slice as JsonObject).owners ?? [],
        },
      }
    : null;

  const isLoading = !noAccess && chartStatus === 'loading';
      ...
      >
        {noAccessError && (
          <div
            ...
            `}
            data-test="chart-access-denied"
          >
            <ErrorMessageWithStackTrace
              error={noAccessError}
              source="dashboard"
              // Not dismissable: there is nothing behind it. Closing would
              // leave a blank tile with no way to get the explanation back.
              closable={false}
            />
          </div>
        )}

        {!noAccess && isLoading && !suppressLoadingSpinner && (
          <ChartOverlay
            style={{
              width,
