...
// A tile the viewer cannot access: the dashboard API answered
// `has_access: false`, so there is no `form_data` behind it. Every menu item
// that would ask for the chart's DATA has to go -- the permission-derived
// flags are instance-wide and cannot know about this tile, so a viewer who
// may generally view queries was offered "View query" on a chart they
// cannot open, and clicking it threw `Cannot read properties of undefined
// (reading 'split')` instead of refusing.
...
test('an inaccessible chart offers no data actions, however permissioned the viewer', () => {
  (global as any).featureFlags = {
    [FeatureFlag.DrillToDetail]: true,
  };
  const props = createProps();
  props.noAccess = true;
  // Explore is already gated on the tile's access by the caller; the point
  // here is the three flags that are NOT.
  props.supersetCanExplore = false;
  renderWrapper(props, ALL_DATA_PERMS);
  openMenu();
...
test('an inaccessible chart cannot be force-refreshed into a doomed request', () => {
  const props = createProps();
  props.noAccess = true;
  renderWrapper(props, ALL_DATA_PERMS);
  openMenu();

  expect(screen.getByTestId('refresh-chart-menu-item')).toHaveClass(
    'ant-dropdown-menu-item-disabled',
  );
});

test('the same viewer keeps every data action on a chart they can access', () => {
  (global as any).featureFlags = {
    [FeatureFlag.DrillToDetail]: true,
  };
  const props = createProps();
  props.noAccess = false;
  props.supersetCanExplore = false;
  renderWrapper(props, ALL_DATA_PERMS);
  openMenu();
