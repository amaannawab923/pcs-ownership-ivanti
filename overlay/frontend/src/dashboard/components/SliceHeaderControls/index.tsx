...
const SliceHeaderControls = (
  ...
  // The tile's own access decides these, not the instance-wide permission:
  // a viewer who may generally view queries still has no query to view on a
  // chart they cannot open (see `noAccess` on the props).
  const canDrillToDetail = permissions.canDrillToDetail && !props.noAccess;
  const canViewQuery = permissions.canViewQuery && !props.noAccess;
  const canViewTable = permissions.canViewTable && !props.noAccess;
  ...
  const newMenuItems: MenuItem[] = [
    {
      ...
      // Refreshing asks for data this viewer will never be given.
      disabled: props.chartStatus === 'loading' || props.noAccess === true,
