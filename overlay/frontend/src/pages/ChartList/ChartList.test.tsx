...
// Owner or Sharing column, no row Sharing action, no drawer.
const ownershipListRoute = 'glob:*/api/v1/ownership/charts*';
...
test('makes no ownership request and shows no ownership surface when OBJECT_OWNERSHIP is off', async () => {
  ...
  try {
    ...
    expect(screen.queryByTestId('chart-row-share')).not.toBeInTheDocument();
    expect(
...
test('fetches ownership once and shows the ownership surface when OBJECT_OWNERSHIP is on', async () => {
  ...
  try {
    ...
    expect(screen.getAllByTestId('chart-row-share').length).toBeGreaterThan(0);
    await waitFor(() => {
