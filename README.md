...
```
module (superset_ownership):     63 files  (incl. configure.py, dashboard_patch.py, plugin_hooks.py,
                                             tests, 7 migrations, 2 model, 1 UPDATING.md; shipped as the wheel)
frontend:                        19 files  (9 replace: 7 core files incl. 2 superset-ui-core, 2 list-page tests;
                                             10 new under src/features/ownership/ + the error-message component)
...
---------------------------------------
overlay total:                   101 files
config layer (generated, not in the diff): 2 files (the 1-line superset_config_ownership.py shim, the .example.py)
UPSTREAM_SHA entries:             11  (the 9 frontend replace-files + the 2 reference-only config/entrypoint sources)
```
